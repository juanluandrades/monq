...
Worker.prototype.poll = function () {
    ...
    this.dequeue(function (err, job) {
        ...
        if (job) {
            ...
            self.jobsTimer[self.queueName + self.jobsCounter] = setTimeout(function() {
                self.work(job);
            }, 0);
            self.jobsCounter++;
        } else {
...
Worker.prototype.killJob = function (queueName, index) {
    var self = this;
    if (self.jobsTimer[self.queueName + self.jobsCounter]) {
        clearTimeout(self.jobsTimer[self.queueName + self.jobsCounter])
    }
};
...
Worker.prototype.work = function (job) {
    ...
    function done(err, result) {
        ...
        clearTimeout(timer);
        delete self.jobsTimer[self.queueName + self.jobsCounter];
