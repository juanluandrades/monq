...
Worker.prototype.poll = function () {
    ...
    this.dequeue(function (err, job) {
        if (err) return self.emit('error', { queue: self.queueName, job: job.data, error: err, cause:'jobDequeue' });
    ...
    function done(err, result) {
        ...
        if (err) {
            self.error(job, err, function (err) {
                if (err) return self.emit('error', { queue: self.queueName, job: job.data, error: err, cause:'jobDone' });
            ...
            job.complete(result, function (err) {
                if (err) return self.emit('error', { queue: self.queueName, job: job.data, error: err, cause:'jobComplete' });
